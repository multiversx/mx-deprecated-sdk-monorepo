...
class TestnetEnvironment(Environment):
    ...
    def deploy_contract(self, contract, owner, arguments, gas_price, gas_limit, value, chain, version):
        logger.debug("deploy_contract")
        proxy = self._get_proxy()
        tx_hash, address = contract.deploy(proxy, owner, arguments, gas_price, gas_limit, value, chain, version)
        return tx_hash, contract.address

    def execute_contract(self, contract, caller, function, arguments, gas_price, gas_limit, value, chain, version):
        logger.debug("execute_contract: %s", contract.address.bech32())
        proxy = self._get_proxy()
        tx_hash = contract.execute(proxy, caller, function, arguments, gas_price, gas_limit, value, chain, version)
        return tx_hash

    def upgrade_contract(self, contract, caller, arguments, gas_price, gas_limit, value, chain, version):
        logger.debug("upgrade_contract: %s", contract.address.bech32())
        proxy = self._get_proxy()
        tx_hash = contract.upgrade(proxy, caller, arguments, gas_price, gas_limit, value, chain, version)
        return tx_hash
