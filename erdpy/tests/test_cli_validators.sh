...
testAll() {
    ...
    ${ERDPY} --verbose stake --pem="${KEYS}/alice.pem" --value="2500000${DENOMINATION}" --number-of-nodes=1 --nodes-public-keys=${BLS_KEY} --reward-address=${REWARD_ADDRESS} --proxy=${PROXY} --estimate-gas --recall-nonce
    ${ERDPY} --verbose stake --pem="${KEYS}/alice.pem" --value="2500000${DENOMINATION}" --number-of-nodes=1 --nodes-public-keys=${BLS_KEY} --reward-address=${REWARD_ADDRESS} --proxy=${PROXY} --estimate-gas --nonce=42
